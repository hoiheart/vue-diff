...
const renderLines = (mode, prev, current) => {
    ...
    diffsMap.map((diffs) => {
        if (diffs.length > 1)
            return; // Return if has index 0, 1
        const type = getDiffType(diffs[0]);
        if (type === 'added') {
...
const renderWords = (prev, current) => {
     ...
     */
    return diffWords(prev, current).filter(word => getDiffType(word) !== 'removed').map(word => {
 ...
 */
const setHighlightCode = ({ highlightCode, language, code }) => {
