...
function watchWithFilter(source, cb, options = {}) {
  const {
    eventFilter = bypassFilter
        ...
        watchOptions = __rest$1(options, ["eventFilter"]);
...
function debouncedWatch(source, cb, options = {}) {
  const {
    debounce = 0
        ...
        watchOptions = __rest$1(options, ["debounce"]);

  return watchWithFilter(source, cb, Object.assign(Object.assign({}, watchOptions), {
...
function css(hljs) {
  ...
  return {
    ...
    }, // attribute values
    {
      ...
      contains: [modes.BLOCK_COMMENT, modes.HEXCOLOR, modes.IMPORTANT, modes.CSS_NUMBER_MODE, ...STRINGS, // needed to highlight these as strings and to avoid issues with
...
const LITERALS$1 = ["true", "false", "null", "undefined", "NaN", "Infinity"]; // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects

const TYPES$1 = [// Fundamental objects
...
"Intl", // WebAssembly
"WebAssembly"];
const ERROR_TYPES$1 = ["Error", "EvalError", "InternalError", "RangeError", "ReferenceError", "SyntaxError", "TypeError", "URIError"];
...
function javascript$1(hljs) {
  ...
  const FUNCTION_CALL = {
    match: regex.concat(/\b/, noneOf([...BUILT_IN_GLOBALS$1, "super"]), IDENT_RE$1$1, regex.lookahead(/\(/)),
  ...
  const FUNCTION_VARIABLE = {
    match: [/const|var|let/, /\s+/, IDENT_RE$1$1, /\s*/, /=\s*/, /(async\s*)?/, // async is optional
...
const getSplitLines = diffsMap => {
  const result = []; // Array(0): prev, Array(1): current
...
const setHighlightCode = ({
  ...
  if (!hasModifiedTags) {
    highlightCode.value = core.highlight(code, {
      language
    }).value;
    return;
  }
  ...
  pureElement.innerHTML = core.highlight(pureCode, {
    language
  }).value; // Highlight DOM without modified tags
...
var script = defineComponent({
  ...
  setup(props) {
    ...
    const {
      minHeight
    } = useVirtualScroll(props, viewer, scrollOptions, meta);
